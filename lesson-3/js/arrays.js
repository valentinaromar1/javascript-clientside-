...
output.textContent = `the thrid element in the array is ${String(myArray[2])}, which is a ${typeof(myArray[2])}!`;
...
output.textContent = `my array has ${Array.length} elements inside`;
// In particular, looping through arrays
for (let i = 0; i < myArray.length; i++){
    //check to see if element is an array
    if (myArray.isArray(myArray[i])){
...
let orig6 = "troronto maple leafs, chicago blackhawks, detroit red wings, boston bruins, montreal canadians, new york rangers";
...
output.textContent = `the last time in the array is ${orig6Array.at(-1)}.`;
...
let numItems = orig6Array.push("buffalo sabres");
...
let numItems = orig6Array.unshift("winnipeg jets");
...
orig6Array.splice(3,1, "edmonton oilers")
